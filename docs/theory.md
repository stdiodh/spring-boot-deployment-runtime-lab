...
- 실습자가 기억해야 할 핵심은 "운영 값은 코드에 하드코딩하지 않는다"입니다.
...
- 실습자가 기억해야 할 핵심은 "Dockerfile은 앱 실행 환경을 문서처럼 고정하는 파일"이라는 점입니다.
- 파일: `Dockerfile`
...
- 실습자가 기억해야 할 핵심은 "CI/CD 파일 안에서도 비밀값은 직접 적지 않는다"입니다.
