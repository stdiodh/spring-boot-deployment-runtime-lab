...
- [ ] 이번 시퀀스는 운영 실행 단위를 만드는 단계이며 CI/CD 전략 전체를 완성하지 않습니다.
...
- 통과 기준: 멘티가 jar, Dockerfile, image, container, log check의 순서를 설명합니다.
- 보완 필요 기준: Docker build만 성공하고 환경변수 분리나 로그 확인을 완료 기준에서 빼고 있습니다.
- 질문 예시: "배포 명령은 성공했는데 애플리케이션이 죽어 있다면 어디를 먼저 보나요?"
- 비교 포인트: 리뷰 단계에서는 jar 경로, prod profile, secret 주입 위치, 로그 확인 명령을 순서대로 봅니다.
